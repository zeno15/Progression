...
namespace System {
	...
	bool InputImplementationWin32::isKeyDown(Keyboard::Key _key) {
		return (GetAsyncKeyState(zenoToSystem(_key)) & 0x8000) != 0;
	}

	bool InputImplementationWin32::isMouseButtonDown(Mouse::Button _button) {
		int vkey = 0;
		switch (_button) {
	...
	Vector2<int> InputImplementationWin32::getMousePosition(void) {
		POINT point;
		GetCursorPos(&point);
	...
	int InputImplementationWin32::zenoToSystem(Keyboard::Key _zenoKey) {
		switch (_zenoKey) {
