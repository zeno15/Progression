...
namespace Graphics {
	...
	bool ImageLoader::loadImageFromFile(const std::string& _filename, std::vector<uint8_t>& _pixels, System::Vector2<unsigned int>& _size) {
		_pixels.clear();
		...
		if (ptr && width && height) {
			...
			std::cerr << "Failed to load image '" << _filename << "', error: " << stbi_failure_reason() << std::endl;

			return false;
		}
	}

	bool ImageLoader::saveImageToFile(const std::string& _filename, const std::vector<uint8_t>& _pixels, const System::Vector2<unsigned int>& _size) {
		...
		std::cerr << "Failed to save image: '" << _filename << "'" << std::endl;
		return false;
	}
