...
namespace Graphics {
	...
	bool Shader::getLocationOfUniforms(const std::vector<std::string>& _uniforms) {
		bool result = true;
		for (auto& uniform : _uniforms) {
			if (!getLocationOfUniform(uniform)) {
				result = false;
			}
		}
	...
	void Shader::passUniform(const std::string& _name, const System::Vector2<float>& _uniform) {
		for (unsigned int i = 0; i < m_UniformLocations.size(); i += 1) {
		...
		std::cerr << "Pass uniform: " << _name << " failed." << std::endl;
	}
	void Shader::passUniform(const std::string& _name, const System::Vector3<float>& _uniform) {
		for (unsigned int i = 0; i < m_UniformLocations.size(); i += 1) {
			if (m_UniformLocations.at(i).first == _name) {
	...
	void Shader::passUniform(const std::string& _name, const System::Vector4<float>& _uniform) {
		for (unsigned int i = 0; i < m_UniformLocations.size(); i += 1) {
			if (m_UniformLocations.at(i).first == _name) {
	...
	void Shader::passUniform(const std::string& _name, const System::Mat4x4& _uniform) {
		for (unsigned int i = 0; i < m_UniformLocations.size(); i += 1) {
			if (m_UniformLocations.at(i).first == _name) {
